...
class SeparableConv2dG(nn.Module):
  def __init__(self, in_channels, out_channels, kernel_size, bias=False):
      super(SeparableConv2dG, self).__init__()
  ...
  def forward(self, x):
      ...
      out = self.pointwise(F.relu(self.bn1(out)))
      return out
...
class ResBlockG(nn.Module):
  def __init__(self, channel,h,w, upscale = False, _kernel_size = 3, addChannel = 0):
    ...
    self.conv1 = SeparableConv2dG(channel,channel + addChannel, kernel_size = _kernel_size,
                           bias = True)
    self.conv2 = SeparableConv2dG(channel+ addChannel, channel + addChannel, kernel_size = _kernel_size,
                           bias = True)
...
class Generator(nn.Module):
  def __init__(self):
    ...
    self.conv2 = nn.Conv2d(32,3,9,1,4,bias = True)
...
generatorMobile = Generator().to(device)
generatorMobile.apply(__weights__init)
...
emaMobile = EMA(generatorMobile)
checkpoint = torch.load('models\KanonNetE2Mobile_30_rms.pt', map_location = device)
generatorMobile.load_state_dict(checkpoint["netG"])
emaMobile.__dict__ = checkpoint["ema"]
emaMobile.apply(generatorMobile)
generatorMobile.eval()

def generateImageM(inputValues, labels):
  ...
  with torch.no_grad():
    return generatorMobile(inputValues, labels).cpu()


if __name__ == '__main__':
  inputValues, labels = generatePoints(1,128)
  images = generateImageM(inputValues, labels).numpy().transpose(0, 2, 3, 1)
