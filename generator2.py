...
generatorSecond = Generator().to(device)
generatorSecond.apply(__weights__init)
...
emaSecond = EMA(generatorSecond)
checkpoint = torch.load('models\KanonNetE2_30_rms.pt', map_location = device)
generatorSecond.load_state_dict(checkpoint["netG"])
emaSecond.__dict__ = checkpoint["ema"]
emaSecond.apply(generatorSecond)
generatorSecond.eval()

def generateImage2(inputValues, labels):
  ...
  with torch.no_grad():
    return generatorSecond(inputValues, labels).cpu()


if __name__ == '__main__':
  inputValues, labels = generatePoints(1,128)
  images = generateImage2(inputValues, labels).numpy().transpose(0, 2, 3, 1)
